...
import {
  ...
  Phone, Mail, CalendarCheck, ArrowDown, MousePointerClick,
  Handshake, Send, AlertCircle, Trophy, Heart, Flame
} from 'lucide-react';
...
const features = [
  { icon: Users, title: 'Gestão de Clientes Completa', desc: 'Cadastro com CPF, endereço, grupos familiares, crianças de colo, histórico de viagens e importação em massa. Nunca perca um dado.', color: 'from-blue-500 to-cyan-500' },
  ...
  { icon: Bus, title: 'Mapa de Assentos Interativo', desc: '5 layouts de ônibus diferentes (Double Deck, JG Turismo, Deca Turismo e mais). Seleção visual de poltronas por andar.', color: 'from-emerald-500 to-teal-500' },
  { icon: CreditCard, title: 'Financeiro Inteligente', desc: 'Parcelas automáticas, recebimentos, despesas pessoais e empresariais. Alertas de vencimento, comprovantes e relatórios.', color: 'from-violet-500 to-purple-500' },
  { icon: FileText, title: 'Contratos Digitais com Assinatura', desc: 'Geração automática de contratos PDF com assinatura digital validada por CPF e IP. Link compartilhável para o cliente assinar.', color: 'from-rose-500 to-pink-500' },
  { icon: Hotel, title: 'Mapa de Quartos e Hospedagem', desc: 'Distribua hóspedes por quartos com controle de capacidade, tipos de cama (solteiro, casal, beliche) e ocupação visual.', color: 'from-cyan-500 to-blue-500' },
  { icon: Calendar, title: 'Calendário Visual de Viagens', desc: 'Todas as viagens em um calendário interativo com cores por status. Planejamento visual completo do seu mês.', color: 'from-indigo-500 to-violet-500' },
  { icon: BarChart3, title: 'Relatórios e Rentabilidade', desc: 'Análise detalhada de receitas x despesas por viagem, margem de lucro, gráficos comparativos e exportação de dados.', color: 'from-pink-500 to-rose-500' },
  { icon: ClipboardList, title: 'Check-in e Lista de Embarque', desc: 'Lista digital com confirmação de presença, local de embarque personalizado e status em tempo real no dia da viagem.', color: 'from-orange-500 to-red-500' },
  { icon: Receipt, title: 'Recibos PDF Automáticos', desc: 'Gere recibos profissionais em PDF com dados do pagamento, cliente e viagem. Download instantâneo com 1 clique.', color: 'from-teal-500 to-emerald-500' },
  { icon: MessageSquare, title: 'WhatsApp Integrado', desc: 'Envie mensagens diretas para clientes via WhatsApp com templates prontos para cobrança, confirmação e lembretes.', color: 'from-green-500 to-emerald-500' },
  { icon: Shield, title: 'Segurança e Controle de Acesso', desc: 'Roles (Admin/Funcionário), permissões granulares, logs de auditoria completos e autenticação segura com Supabase.', color: 'from-slate-500 to-gray-600' },
];
...
const faqs = [
  ...
  { q: 'Tem período de teste gratuito?', a: 'Sim! Oferecemos um teste completo e gratuito para você conhecer todas as funcionalidades antes de decidir. Sem cartão de crédito, sem compromisso.' },
  { q: 'A demonstração é realmente gratuita?', a: 'Totalmente! Agendamos uma reunião online onde mostramos o sistema completo funcionando com dados reais. Você não paga nada e decide com calma.' },
];
...
const testimonials = [
  ...
  { name: 'Fernanda R.', role: 'Operadora de Turismo - RJ', text: 'Cheguei a testar 3 sistemas antes e nenhum era feito pra turismo rodoviário de verdade. O Agência Sistema entende o que a gente precisa.', stars: 5 },
];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <Input
        ...
        className={isHero ? 'h-12 bg-white/10 border-white/20 text-white placeholder:text-white/50' : 'h-12'}
      />
      <Input
        placeholder="WhatsApp (ex: 81 99999-9999)"
        required
        value={form.telefone}
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3 items-end">
      <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer">
        <Button className="h-14 px-6 rounded-full bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white shadow-2xl shadow-emerald-500/30 text-base font-bold animate-pulse hover:animate-none">
          <MessageSquare className="w-5 h-5 mr-2" /> Teste Grátis
        </Button>
      </a>
    </div>
  );
...
export default function LandingPage() {
  ...
  const navItems = ['Funcionalidades', 'Módulos', 'Comparativo', 'Depoimentos', 'FAQ'];
    ...
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="md:hidden bg-background border-t border-border px-4 py-4 space-y-3">
            ...
            <a href="#agendar" onClick={() => setMobileMenu(false)}>
              <Button className="w-full bg-gradient-to-r from-emerald-500 to-green-600 text-white">
                <CalendarCheck className="w-4 h-4 mr-2" /> Agendar Demo Grátis
              </Button>
            </a>
          </div>
        )}
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-6 mb-8">
                {[
                  ...
                  { icon: CheckCircle2, text: 'Demo personalizada' },
              ...
              <div className="lg:hidden mb-8">
                <img src={mockupSistema} alt={`Dashboard ${BRAND}`} className="rounded-2xl shadow-2xl border border-white/10" />
              </div>
            </div>

            {/* Formulário no Hero */}
            <div className="relative" id="agendar">
              ...
              <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8">
                <div className="text-center mb-6">
                  ...
                  <p className="text-blue-200/70 text-sm">Veja o sistema funcionando ao vivo, sem compromisso</p>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap justify-center items-center gap-8 md:gap-16 text-white">
            {[
              ...
              { value: '200+', label: 'Agências Testaram' },
              { value: '99.9%', label: 'Uptime Garantido' },
              { value: '5★', label: 'Nota dos Clientes' },
            ].map((stat, i) => (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Não é só uma apresentação — é uma consultoria personalizada para a sua agência
            </p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: Monitor, title: 'Demo Ao Vivo', desc: 'Veja o sistema funcionando com dados reais de viagens, clientes e financeiro', color: 'from-blue-500 to-cyan-500' },
              { icon: Handshake, title: 'Consultoria Grátis', desc: 'Nossa equipe analisa seu fluxo atual e mostra como otimizar com o sistema', color: 'from-amber-500 to-orange-500' },
              { icon: Settings, title: 'Setup Personalizado', desc: 'Configuramos o sistema com sua marca, logo e cores — pronto pra usar', color: 'from-emerald-500 to-green-500' },
              { icon: HeadphonesIcon, title: 'Suporte Dedicado', desc: 'Acompanhamento nos primeiros dias para garantir que tudo funcione perfeito', color: 'from-violet-500 to-purple-500' },
            ].map((item, i) => (
              <Card key={i} className="border-border text-center hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
          ...
          <div className="text-center mt-12">
            <a href="#agendar">
              <Button size="lg" className="bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white shadow-xl h-14 text-lg px-10 font-bold">
                <Rocket className="w-5 h-5 mr-2" /> Quero Resolver Esses Problemas — Agendar Demo
              </Button>
            </a>
            <p className="text-xs text-muted-foreground mt-3">Demonstração gratuita • Sem compromisso • Leva 30 min</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-gradient-to-r from-blue-600 to-cyan-500 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold mb-4">
            Quer ver o sistema funcionando ao vivo?
          </h2>
          <p className="text-xl text-white/90 mb-8 max-w-2xl mx-auto">
            Agende uma reunião gratuita de 30 minutos. Mostramos o sistema completo, tiramos suas dúvidas e <strong>configuramos tudo na hora</strong>.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-6 mt-8 text-sm text-white/70">
            <span className="flex items-center gap-1"><CheckCircle2 className="w-4 h-4 text-emerald-300" /> 100% gratuito</span>
            <span className="flex items-center gap-1"><CheckCircle2 className="w-4 h-4 text-emerald-300" /> Sem compromisso</span>
            <span className="flex items-center gap-1"><CheckCircle2 className="w-4 h-4 text-emerald-300" /> Via Google Meet</span>
          </div>
        </div>
      </section>
          ...
          <div className="text-center mt-8">
            <a href="#agendar">
              <Button size="lg" className="bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white shadow-lg h-12 px-8 font-bold">
                <Trophy className="w-5 h-5 mr-2" /> Testar o Sistema Mais Completo — É Grátis
              </Button>
            </a>
          </div>
        </div>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold mb-4">
              Da demonstração ao uso em 4 passos
            </h2>
          </div>
          <div className="grid md:grid-cols-4 gap-8">
            {[
              { step: '01', icon: CalendarCheck, title: 'Agende a Demo', desc: 'Preencha o formulário e nossa equipe agenda uma reunião em até 2h.', color: 'from-emerald-500 to-green-500' },
              { step: '02', icon: Monitor, title: 'Veja Funcionando', desc: 'Mostramos o sistema ao vivo com dados reais e tiramos todas as suas dúvidas.', color: 'from-blue-500 to-cyan-500' },
              { step: '03', icon: Settings, title: 'Configuramos Tudo', desc: 'Personalizamos com sua marca, logo, cores e importamos seus clientes.', color: 'from-amber-500 to-orange-500' },
              { step: '04', icon: Rocket, title: 'Comece a Usar', desc: 'Acesso liberado com suporte dedicado nos primeiros dias. Pronto pra lucrar mais!', color: 'from-violet-500 to-purple-500' },
            ].map((item, i) => (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <a href="#agendar">
                <Button size="lg" className="bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white shadow-lg h-12 px-8 font-bold">
                  <Play className="w-5 h-5 mr-2" /> Ver Tudo Isso Funcionando — Grátis
                </Button>
              </a>
            </div>
          ...
          <div className="text-center mt-8">
            ...
            <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer">
              <Button variant="outline" className="h-12 px-6 font-semibold">
                <MessageSquare className="w-4 h-4 mr-2" /> Perguntar no WhatsApp
              </Button>
            </a>
          </div>
        </div>
      ...
      <section className="py-16 bg-gradient-to-r from-blue-600 to-cyan-500">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
          ...
          <p className="text-xl text-white/90 mb-6">
            Estamos aceitando apenas <strong>10 novas agências por mês</strong> para garantir o setup personalizado e suporte dedicado para cada cliente.
          </p>
          <a href="#agendar">
            <Button size="lg" className="bg-white text-blue-600 hover:bg-white/90 shadow-xl text-lg px-10 h-14 font-bold">
              <CalendarCheck className="w-5 h-5 mr-2" /> Garantir Minha Vaga — Agendar Demo
            </Button>
          </a>
          <p className="text-sm text-white/60 mt-4">Demonstração 100% gratuita e sem compromisso</p>
        </div>
      </section>

      {/* Final CTA — Formulário grande */}
              ...
              <div className="grid lg:grid-cols-2 gap-12 items-center">
                <div>
                  ...
                  <p className="text-lg text-muted-foreground mb-6">
                    Agende sua demonstração gratuita agora. Em 30 minutos você vai entender por que agências estão migrando para o {BRAND}.
                  </p>
                  <div className="space-y-4">
                    {[
                      { icon: CalendarCheck, text: 'Demonstração ao vivo de 30 minutos' },
                      { icon: Settings, text: 'Setup personalizado com sua marca incluso' },
                      { icon: HeadphonesIcon, text: 'Suporte dedicado nos primeiros dias' },
                      { icon: Gift, text: 'Teste gratuito sem cartão de crédito' },
                    ].map((item, i) => (
                ...
                <div>
                  ...
                  <div className="mt-6 text-center">
                    <p className="text-xs text-muted-foreground mb-3">Ou se preferir, fale direto conosco:</p>
