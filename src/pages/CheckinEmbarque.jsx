...
import { CheckCircle2, Circle, Users, MapPin, Search, Plane, ClipboardList } from "lucide-react";
...
export default function CheckinEmbarque() {
  ...
  const { data: clientes = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("clientes")
        .select("id, nome_completo, cpf, telefone, poltrona, local_embarque, status_pagamento, observacoes, e_crianca_colo, cor_grupo, numero_grupo")
  ...
  // Usamos campo observacoes com prefixo especial para marcar embarque
  // mas para não adicionar coluna nova, vamos usar um estado local salvo via localStorage
  ...
  React.useEffect(() => {
    if (viagemSelecionada) {
      try {
        setEmbarcados(JSON.parse(localStorage.getItem(`checkin_${viagemSelecionada}`) || "{}"));
      } catch { setEmbarcados({}); }
    }
  }, [viagemSelecionada]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Controle de presença por viagem</p>
      </div>
...
function ClienteCheckinRow({ cliente, embarcado, onToggle, getCorDisplay }) {
  return (
    <div
